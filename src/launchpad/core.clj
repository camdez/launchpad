...
(def modes
  {:paint   {:on-tap  paint
             :on-self (fn [{:keys [board old-board] :as state}]
                        (if (and old-board (= board empty-board)) ; undo
                          (assoc state
                                 :board old-board)
                          (assoc state  ; clear
                                 :old-board board
  ...
  (when (= :note-off command)
    (cond
      ...
      (when-let [new-mode (let [idx (int (/ (- note 8) 16))]
        ...
        (let [{:keys [on-enter on-self]} (get modes new-mode)
              {:keys [mode]}             @state-atom
              {:keys [on-leave]}         (get modes mode)
...
(defn run [in]
  (let [exit? (promise)
    ...
    (try
      ...
      (swap! state #(assoc % :board exit-board))
      (finally
...
;; This would make a nice, basic animation example.
